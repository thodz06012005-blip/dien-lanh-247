...
test('Phase 14 validates and normalizes all request DTO failures', () => {
  ...
  const factory = read('backend/src/common/validation/validation-exception.factory.ts');
  for (const marker of [
    'whitelist: true',
...
test('Phase 14 enforces backend RBAC instead of frontend-only hiding', () => {
  const operations = read('backend/src/modules/operations/operations.controller.ts');
  ...
  for (const source of [operations, serviceRequests, adminAuth]) {
    assert.match(source, /JwtAuthGuard/);
    assert.match(source, /RolesGuard/);
    assert.match(source, /@Roles\(/);
  }
  assert.match(operations, /PermissionsGuard/);
  assert.match(operations, /@Permissions\(/);
...
test('Phase 14 protects login refresh and session revocation', () => {
  ...
  const limiter = read('backend/src/modules/auth/login-rate-limit.service.ts');
...
test('Phase 14 keeps destructive operations recoverable and confirmed', () => {
  const controller = read('backend/src/modules/technicians/technicians.controller.ts');
  const service = read('backend/src/modules/technicians/technicians.service.ts');
  assert.match(controller, /x-confirm-dangerous-action/i);
  assert.match(controller, /DANGEROUS_ACTION_BLOCKED/);
  assert.match(controller, /TECHNICIAN_SOFT_DELETED/);
  assert.match(service, /status: TechnicianStatus\.inactive/);
});
