...
function initialState(record?: CmsRecord | null): FormState {
  ...
  const fields = [
    ...
    'content','description','warranty','quote','bio','result','evidenceReference','verificationReference','pricing','process','faq','tasks','config',
  ...
  state.imageRightsConfirmed = asText(source.imageRightsConfirmed ?? false);
  state.isVerified = asText(source.isVerified ?? false);
...
export default function CmsEditorDrawer({ type, initial, saving, error, onClose, onSave }: CmsEditorDrawerProps) {
  ...
  const buildPayload = () => {
    ...
    if (type === 'projects') Object.assign(payload, { clientName: form.clientName || undefined, location: form.location || undefined, startedAt: form.startedAt || undefined, completedAt: form.completedAt || undefined, tasks: parseJson(form.tasks, 'Nhiệm vụ'), result: form.result || undefined, evidenceReference: form.evidenceReference || undefined, imageRightsConfirmed: form.imageRightsConfirmed === 'true', mediaIds: parseIds(form.mediaIds) });
    ...
    if (type === 'testimonials') Object.assign(payload, { customerName: form.customerName, customerTitle: form.customerTitle || undefined, company: form.company || undefined, quote: form.quote, rating: numberValue(form.rating) ?? 5, avatarMediaId: numberValue(form.avatarMediaId), serviceId: numberValue(form.serviceId), isVerified: form.isVerified === 'true', verificationReference: form.verificationReference || undefined });
      ...
      <section className="relative flex h-full w-full max-w-5xl flex-col overflow-hidden bg-[#F5F7FB] shadow-2xl">
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-7">
          ...
          {tab === 'content' && <div className="space-y-6">
            ...
            {type === 'projects' && <section className="grid gap-5 rounded-[2rem] border border-slate-200 bg-white p-5 sm:grid-cols-2"><Field label="Khách hàng"><Input value={form.clientName} onChange={(value) => setField('clientName', value)} /></Field><Field label="Địa điểm"><Input value={form.location} onChange={(value) => setField('location', value)} /></Field><Field label="Nhiệm vụ JSON"><Textarea value={form.tasks} onChange={(value) => setField('tasks', value)} /></Field><Field label="Kết quả"><Textarea value={form.result} onChange={(value) => setField('result', value)} /></Field><Field label="Nguồn bằng chứng" required hint="Mã yêu cầu, biên bản nghiệm thu hoặc hồ sơ nội bộ có thể đối chiếu."><Textarea value={form.evidenceReference} onChange={(value) => setField('evidenceReference', value)} /></Field><Field label="Quyền sử dụng ảnh" required><Select value={form.imageRightsConfirmed} onChange={(value) => setField('imageRightsConfirmed', value)} options={[{value:'false',label:'Chưa xác nhận — không xuất bản'},{value:'true',label:'Đã có quyền sử dụng'}]} /></Field></section>}
            ...
            {type === 'testimonials' && <section className="grid gap-5 rounded-[2rem] border border-slate-200 bg-white p-5 sm:grid-cols-2"><Field label="Chức danh"><Input value={form.customerTitle} onChange={(value) => setField('customerTitle', value)} /></Field><Field label="Công ty"><Input value={form.company} onChange={(value) => setField('company', value)} /></Field><Field label="Đánh giá" required><Textarea value={form.quote} onChange={(value) => setField('quote', value)} /></Field><Field label="Số sao"><Input type="number" value={form.rating} onChange={(value) => setField('rating', value)} /></Field><Field label="Nguồn xác minh" required hint="Mã yêu cầu hoặc hồ sơ đồng ý công bố có thể đối chiếu."><Textarea value={form.verificationReference} onChange={(value) => setField('verificationReference', value)} /></Field><Field label="Trạng thái xác minh" required><Select value={form.isVerified} onChange={(value) => setField('isVerified', value)} options={[{value:'false',label:'Chưa xác minh — không hiển thị'},{value:'true',label:'Đã xác minh'}]} /></Field></section>}
